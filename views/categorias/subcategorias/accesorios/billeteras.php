...
try {
    ...
    $productos = $modeloProducto->obtenerCatalogo('Accesorios', 'Billetera');
} catch (Exception $e) {
    $productos = [];
}
?>
...
<section class="container my-5">
    ...
    <a href="../../accesorios.php" class="btn btn-outline-dark btn-volver position-absolute start-0" style="font-weight: 600; font-size: 1.1rem; padding: 10px 20px; z-index: 10;">
        <i class="bi bi-arrow-left"></i> Volver
    </a>
    <h2 class="text-center m-0">Billeteras</h2>
</div>
                ...
                <div class="col">
                    <div class="card h-100 product-card open-product-modal" style="cursor: pointer;" data-desc="<?php echo htmlspecialchars($p['descripcion'] ?? 'Sin descripción'); ?>" data-brand="<?php echo htmlspecialchars($p['marca'] ?? 'Desconocida'); ?>" data-stock="<?php echo $p['stock']; ?>">
                        <div class="p-3 d-flex justify-content-center align-items-center" style="height: 250px;">
                            <img src="/controlux/<?php echo htmlspecialchars(ltrim($p['img'] ?? 'img/accesorio.png', '/')); ?>" class="card-img-top" alt="<?php echo htmlspecialchars($p['nombre']); ?>" style="max-height: 100%; width: auto; object-fit: contain;">
                        </div>
                        <div class="card-body d-flex flex-column text-center">
                            ...
                            <?php if($p['stock'] > 0): ?>
                                <button class="btn btn-gold w-100 mt-auto add-to-cart-btn" 
                                        ...
                                        data-img="/controlux/<?php echo htmlspecialchars(ltrim($p['img'] ?? 'img/accesorio.png', '/')); ?>">

...
try {
    ...
    $productos = $modeloProducto->obtenerCatalogo('Relojes', 'Patek');
} catch (Exception $e) {
    $productos = [];
}
?>
...
<section class="container my-5 position-relative">
    <a href="../altagama.php" class="btn btn-outline-dark position-absolute" style="top: 0; left: 15px; font-weight: 600; font-size: 1.1rem; padding: 10px 20px;">
        <i class="bi bi-arrow-left"></i> Volver
    </a>
    
    <h2 class="text-center mb-5">Patek Philippe</h2>
                ...
                <div class="col">
                    <div class="card h-100 product-card open-product-modal" style="cursor: pointer;" data-desc="<?php echo htmlspecialchars($p['descripcion'] ?? 'Sin descripción'); ?>" data-brand="<?php echo htmlspecialchars($p['marca'] ?? 'Desconocida'); ?>" data-stock="<?php echo $p['stock']; ?>">
                        <div class="p-3 d-flex justify-content-center align-items-center" style="height: 250px;">
                            <img src="/controlux/<?php echo htmlspecialchars(ltrim($p['url_imagen'] ?? 'img/relojes/PatekPhilippe-Aquanaut.webp', '/')); ?>" class="card-img-top" alt="<?php echo htmlspecialchars($p['nombre']); ?>" style="max-height: 100%; width: auto; object-fit: contain;">
                        </div>
                        <div class="card-body d-flex flex-column text-center">
                            ...
                            <?php if($p['stock'] > 0): ?>
                                <button class="btn btn-gold w-100 mt-auto add-to-cart-btn" 
                                        ...
                                        data-img="/controlux/<?php echo htmlspecialchars(ltrim($p['url_imagen'] ?? 'img/relojes/PatekPhilippe-Aquanaut.webp', '/')); ?>">
